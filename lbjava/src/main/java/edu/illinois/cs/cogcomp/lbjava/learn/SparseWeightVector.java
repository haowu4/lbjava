 ...
 * associated with <code>Double</code>s and/or with <code>double[]</code>s representing the weights
...
public class SparseWeightVector implements Cloneable, Serializable {
     ...
     * in this weight vector and the result is returned as an array of arrays. The first array
     * contains the integer keys of the example's features, as indexed in the lexicon. The second
     * array gives the double values corresponding to the product of the pairwise multiplication of
     ...
     * annotation, ends with an <code>"End"</code> annotation, and without a <code>Lexicon</code>
     * passed as a parameter, the weights are simply printed in the order of their integer indices.
     *
    ...
    /**
     * Outputs a textual representation of this <code>SparseWeightVector</code> to a stream just
     * like {@link #write(PrintStream)}, but without the <code>"Begin"</code> and <code>"End"</code>
     * annotations. Without a <code>Lexicon</code> passed as a parameter, the weights are simply
     ...
     * returns the data computed by {@link #write(PrintStream)}.
     *
    ...
    /**
     * Reads the binary representation of a weight vector of any type from the given stream. The
     ...
     **/
    public static SparseWeightVector readWeightVector(ExceptionlessInputStream in) {
        ...
        try {
            result = (SparseWeightVector) c.newInstance();
        } catch (Exception e) {
    ...
    /**
     * Returns a copy of this <code>SparseWeightVector</code> in which the {@link #weights} variable
    ...
    public Object clone() {
        ...
        try {
            clone = (SparseWeightVector) super.clone();
        } catch (Exception e) {
